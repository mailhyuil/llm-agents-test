...
import { evaluatorLlmWithOutput } from "../models/evaluator";
...
export async function evaluate(
  state: typeof EcommerceAnnotation.State,
): Promise<Partial<typeof EcommerceAnnotation.State>> {
  ...
  try {
    const evalResult = await evaluatorLlmWithOutput.invoke(evaluatorMessages, config);

    const newMessages: AIMessage[] = [new AIMessage({ content: `평가자 피드백: ${evalResult.feedback}` })];

    const newState: Partial<typeof EcommerceAnnotation.State> = {
      ...state,
